...
r.list = sample(1:nrow(AM4.top.1),61,replace=FALSE)
...
r.list = sample(1:nrow(AM4.top.2),61,replace=FALSE)
...
r.list = sample(1:nrow(AM4.top.3),61,replace=FALSE)
...
r.list = sample(1:nrow(AM4.top.4),46,replace=FALSE)
...
AM4.top.5 = filter(AM4.top,var=="five")
r.list = sample(1:nrow(AM4.top.5),61,replace=FALSE)
distList.5 = sapply(r.list,FUN=function(r){
  
  AM4.comp = slice(AM4.top.5,-r)
...
obsDist.5 = mean(AM4.top.5$Distance)
...
p5 = round(ks.test(obsDist.5,distList.5$Distance,alternative=c("two.sided"))$p.value,2)

# Make dataframes for plotting histograms (in style of Figure 2 in McManus et al. Appendix)
obsDist.df = as.data.frame(matrix(nrow=5,ncol=3))
obsDist.df[,1] = c(obsDist.1,obsDist.2,obsDist.3,obsDist.4,obsDist.5)
obsDist.df[,2] = c("De Novo","0.1%","1%","10%","25%")
obsDist.df[,3] = c(p1,p2,p3,p4,p5)
...
if (plot.opt == "yes"){
  ...
  ggplot()+geom_histogram(data=distList.df,aes(Distance, stat(density)),fill=NA,col="black")+
    ...
    geom_text(data=obsDist.df,aes(x=3,y=2,label=paste("p =",p,sep=" ")),size=4)+
    ylab("Density")+
    facet_wrap(var~.)+
    theme_classic()+
    theme(
